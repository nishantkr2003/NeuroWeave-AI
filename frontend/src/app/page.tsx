...
const features = [
  {
    title: "Unified media reasoning",
    description:
      "Analyze images, documents, audio, and videos together with seamless AI-powered cross-media understanding.",
  },
  {
    title: "Instant insight delivery",
    description:
      "Generate summaries, extract structured intelligence, compare files, and surface actionable insights in seconds.",
  },
  {
    title: "Persistent session memory",
    description:
      "Maintain full conversational context across uploads, chats, and file selections for uninterrupted productivity.",
  },
];
...
const workflow = [
  ...
  "Extract instant intelligence",
];

const stats = [
  { label: "Media Types Supported", value: "10+" },
  { label: "Insight Speed", value: "<3s" },
  { label: "Context Accuracy", value: "99%" },
];
...
export default function LandingPage() {
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#050507] text-white">
      {/* Background Effects */}
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(99,102,241,0.18),transparent_30%),radial-gradient(circle_at_bottom_left,rgba(0,229,160,0.12),transparent_28%)]" />
      <div className="absolute inset-0 opacity-10 bg-[linear-gradient(to_bottom,white_1px,transparent_1px),linear-gradient(to_right,white_1px,transparent_1px)] bg-[size:80px_80px]" />
      ...
      <header className="relative z-20 mx-auto flex max-w-7xl items-center justify-between px-6 py-8">
        ...
        <nav className="flex items-center gap-4">
          ...
          <Link
            href="/auth/register"
            className="rounded-full bg-indigo-600 px-5 py-2 text-sm font-semibold transition hover:bg-indigo-500"
          >
            Get Started
          </Link>
        </nav>
      ...
      {/* Hero Section */}
      <section className="relative z-10 mx-auto grid max-w-7xl gap-14 px-6 pb-24 pt-12 lg:grid-cols-2 lg:items-center">
        <div>
          <div className="mb-6 inline-flex rounded-full border border-indigo-400/20 bg-indigo-500/10 px-4 py-2 text-xs uppercase tracking-[0.35em] text-indigo-300">
          ...
          <p className="mt-6 max-w-2xl text-lg leading-8 text-white/70">
            NeuroWeave-AI transforms documents, images, videos, and audio into
            unified intelligence. Upload once, analyze instantly, and discover
            connected insights faster than ever.
          </p>

          <div className="mt-8 flex flex-col gap-4 sm:flex-row">
            <Link
              href="/auth/register"
              className="rounded-full bg-indigo-600 px-8 py-4 text-sm font-semibold transition hover:bg-indigo-500"
            >
              Start Free
            </Link>
            ...
            <Link
              href="/auth/login"
              className="rounded-full border border-white/10 bg-white/5 px-8 py-4 text-sm text-white/80 transition hover:bg-white/10"
            >
              Live Demo
            </Link>
          </div>
        ...
        <div className="relative">
          <div className="rounded-[2rem] border border-white/10 bg-white/5 p-8 shadow-[0_40px_120px_rgba(0,0,0,0.45)] backdrop-blur-2xl">
            <div className="space-y-6">
              <div className="rounded-3xl border border-white/10 bg-[#0B1118] p-6">
                <p className="text-xs uppercase tracking-[0.3em] text-cyan-300">
                  Live AI Session
                </p>
                <p className="mt-4 text-sm leading-7 text-white/70">
                  Compare PDFs, videos, voice notes, and images in one
                  continuous workflow with persistent memory.
                </p>
              </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {workflow.map((step, index) => (
                  ...
                  >
                    <span className="mb-2 block text-indigo-300">
                      0{index + 1}
                    </span>
                    {step}
                  </div>
      ...
      <section className="relative z-10 mx-auto max-w-7xl px-6 pb-24">
        <div className="mb-12 text-center">
          ...
          <h2 className="mt-4 text-4xl font-semibold">
            Designed for intelligent workflows
          </h2>
        </div>
        ...
        <div className="grid gap-8 md:grid-cols-3">
          {features.map((feature) => (
            <article
              key={feature.title}
              className="group rounded-[2rem] border border-white/10 bg-white/5 p-8 backdrop-blur-xl transition duration-300 hover:-translate-y-2 hover:border-indigo-400/40"
            >
      ...
      <section className="relative z-10 mx-auto max-w-5xl px-6 pb-24">
        <div className="rounded-[2.5rem] border border-indigo-500/20 bg-gradient-to-r from-indigo-500/10 to-cyan-500/10 p-12 text-center backdrop-blur-xl">
          <h2 className="text-4xl font-semibold">
            Ready to redefine how you work with media?
          </h2>
          <p className="mx-auto mt-4 max-w-2xl text-white/70">
            Join NeuroWeave-AI and unlock a smarter way to analyze, compare, and
            synthesize every file type.
          </p>

          <Link
            href="/auth/register"
            className="mt-8 inline-flex rounded-full bg-indigo-600 px-8 py-4 text-sm font-semibold transition hover:bg-indigo-500"
          >
            Launch Now
          </Link>
        </div>
